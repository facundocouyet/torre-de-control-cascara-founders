...
Cada plantilla sale de un JSON en plantillas/<id>.json con este esquema:
...
if __name__ == "__main__":
    ...
    os.makedirs('plantillas/out', exist_ok=True)
    n = 0
    for f in sorted(glob.glob('plantillas/*.json')):
        pl = json.load(open(f, encoding='utf-8'))
        mid = pl["modulo"]
        if mid not in meta:
            print("módulo desconocido:", mid); continue
        nom, cat = meta[mid]
        open(f'plantillas/out/{mid}.html', 'w', encoding='utf-8').write(render(pl, nom, cat))
        n += 1
    print("plantillas:", n)
