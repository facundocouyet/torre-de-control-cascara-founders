...
if __name__ == "__main__":
    ...
    for c in inv["categorias"]:
        for m in c["modulos"]:
            open(os.path.join(RAIZ, 'cartas', f'{m["id"]}.html'), 'w', encoding='utf-8').write(render(m, c["nombre"]))
            indice.append({"id": m["id"], "categoria": c["nombre"], "nombre": m["nombre"]})
            n += 1
    json.dump(indice, open(os.path.join(RAIZ, 'cartas', 'indice.json'), 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
    print("cartas:", n)
